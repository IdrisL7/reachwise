        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {trustItems.map((item, i) => (
            <div key={i} className="flex items-start gap-3">
              <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-violet-600/10 text-violet-400">
                {item.icon}
              </div>
              <p className="text-[0.8125rem] leading-[1.55] text-zinc-400">
                {item.text}
              </p>
            </div>
          ))}
